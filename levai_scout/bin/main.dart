...
Future<List<SearchResult>?> _handleAiQuery({
  ...
  for (final q in allQueries) {
    try {
      final results = await searxngClient.search(q, maxResults: _kMaxSearchResults);
...
Future<List<SearchResult>> _handleSearchOnlyQuery({
  ...
  final results = await searxngClient.search(query, maxResults: _kMaxSearchResults);
...
Future<bool> _runHealthCheck(
  ...
  stdout.writeln('${AnsiCodes.gray}Executando health check...${AnsiCodes.reset}');
...
Future<AppMode> _promptModeSelection(AppMode current, bool searxngHealthy) async {
  stdout.writeln(
    '${AnsiCodes.cyan}${AnsiCodes.bold}Escolha o modo de operação:'
    '${AnsiCodes.reset}',
  );
  for (final mode in AppMode.values) {
    final marker = mode == current ? '${AnsiCodes.green}●' : '${AnsiCodes.gray}○';
    stdout.writeln(
      '  $marker${AnsiCodes.reset} ${AnsiCodes.bold}${mode.menuIndex}. '
      '${mode.title}${AnsiCodes.reset}',
    );
    stdout.writeln('     ${AnsiCodes.gray}${mode.description}${AnsiCodes.reset}');
  }
  stdout.write(
