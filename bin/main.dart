...
/// navegador de terminal, lendo uma página inteira.
...
// Modelo rápido criado por você.
const String _kOllamaModel = 'qwen-fast';
...
Future<List<SearchResult>?> _handleAiQuery({
  ...
  stdout.writeln(
    '${AnsiCodes.gray}'
    'Gerando sub-buscas relacionadas...'
    '${AnsiCodes.reset}',
  );
  ...
  if (relatedQueries.isNotEmpty) {
    stdout.writeln(
      '${AnsiCodes.gray}'
      'Sub-buscas: ${relatedQueries.join(" | ")}'
      '${AnsiCodes.reset}',
    );
  }
  ...
  stdout.writeln(
    '${AnsiCodes.gray}'
    '${topResults.length} resultado(s) combinados. '
    'Raspando páginas '
    '(até $_kScraperConcurrency simultâneas)...'
    '${AnsiCodes.reset}',
  );

  final urls = topResults.map((result) => result.url).toList(growable: false);
  ...
  if (successfulPages.isEmpty) {
    stdout.writeln(
      '${AnsiCodes.yellow}'
      'Não foi possível extrair conteúdo de '
  ...
  final answer = rawAnswerBuffer.toString();
...
Future<void> _handleOpenCommand({
  ...
  if (asNumber != null) {
    ...
    stdout.writeln(
      '${AnsiCodes.yellow}'
      'Argumento inválido. Use um número da última busca '
...
Future<bool> _runHealthCheck(
  ...
  if (!searxngOk) {
    stdout.writeln(
      '${AnsiCodes.yellow}'
      'Aviso: sem SearXNG, os modos "IA Avançada" '
      'e "Busca Direta" não funcionarão. '
      'Use o Chat Offline enquanto isso.'
      '${AnsiCodes.reset}\n',
    );
  }
